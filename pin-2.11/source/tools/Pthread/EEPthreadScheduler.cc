  ...
  port_num(port_num_), skip_first(0), first_instrs(0), agile_bank_th(0)
{
  ...
  page_sz_log2       = pts->get_param_uint64("pts.l1itlb.page_sz_log2", 13);
...
void PthreadScheduler::PlayTraces(const string & trace_name)
{
  do
  {
    ...
    ADDRINT func, ADDRINT arg)
{
  ...
  if (pthreads.size() > 1)
  {
    ...
    pts->add_instruction(pthreads.size()-1, 0, curr_time, 0, 0, 0, 0, 0, 0, 0, 
...
void PthreadScheduler::UnblockThread(pthread_t thread, bool isbarrier) 
{
  ...
  pts->add_instruction(pth_to_hth[GetThreadPtr(thread)->second], 0, curr_time, 0, 0, 0, 0, 0, 0, 0,
		       false, false, false, false, isbarrier, false, false, false, false,
		       0, 0, 0, 0, 0, 0, 0, 0);
  nactive++;
}
...
{
  pts->add_instruction(pth_to_hth[GetThreadPtr(thread)->second], 0, curr_time, 0, 0, 0, 0, 0, 0, 0,
		       false, false, islock, isunlock, isbarrier, false, false, false,false,
		       0, 0, 0, 0, 0, 0, 0, 0);
}

void PthreadScheduler::add_persism_instruction(pthread_t thread, uint32_t txid_, bool iscommit, bool isflush,
		 bool islog_, bool istx_)
{
//	if (isflush) cout<<"add isflush"<<endl;
  pts->add_instruction(pth_to_hth[GetThreadPtr(thread)->second], txid_, curr_time, 0, 0, 0, 0, 0, 0, 0,
		  	  false, false, false, false, false, istx_, iscommit, islog_,isflush,
      0, 0, 0, 0, 0, 0, 0, 0);
}
    ...
    uint32_t rw0, uint32_t rw1, uint32_t rw2, uint32_t rw3)
{
  ...
  bool must_resume = pts->add_instruction(pth_to_hth[current->second], pthread->txid, curr_time,
					  waddr, wlen, raddr, raddr2, rlen, ip, category,
					  isbranch, isbranchtaken, false, false,
					  false, pthread->istx, false,
					  pthread->islog,false,
					  rr0, rr1, rr2, rr3, rw0, rw1, rw2,
					  rw3, true);
...
Pthread::Pthread(
  ...
  skip_instrs(0), spinning(0), txid(0), istx(false), tx_commit(false), islog(false), isflush(false),
  num_mutex_lock(0), num_mutex_trylock(0)
{
...
void PthreadScheduler::mcsim_tx_begin()
{
	current->second->istx = true;
	current->second->txid++; // pm 10-19
//	current->second->tx_commit = false;
//	add_persism_instruction(current, txid, false, false);
}
void PthreadScheduler::mcsim_tx_end()
{
	current->second->istx = false;
//	current->second->tx_commit = true;
	pthread_t current_ = GetCurrentThread();
	add_persism_instruction(current_, current->second->txid, true, false);
}
void PthreadScheduler::mcsim_log_begin()
{
	current->second->islog = true;
	current->second->txid++;//this is in fact log ID
//	add_persism_instruction(current, txid, false, false);
} // pm baseline 9-4
void PthreadScheduler::mcsim_log_end()
{
//	cout<<"logend flush"<<endl;
	current->second->islog = false;
	pthread_t current_ = GetCurrentThread();
	add_persism_instruction(current_, current->second->txid, false, true);
} // pm baseline 9-4

void PthreadScheduler::mcsim_mem_fence()
{
//	cout<<"logend flush"<<endl;
//	current->second->islog = false;
	pthread_t current_ = GetCurrentThread();
	add_persism_instruction(current_, current->second->txid, true, true);
}
