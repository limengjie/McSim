...
VOID FlagImg(IMG img, VOID* v) 
{
  ...
  rtn = RTN_FindByName(img, "mcsim_tx_begin");
  if (rtn != RTN_Invalid())
  {
    RTN_Replace(rtn, (AFUNPTR)CallMcSimTxBegin);
  }
  rtn = RTN_FindByName(img, "mcsim_tx_end");
  if (rtn != RTN_Invalid())
  {
    RTN_Replace(rtn, (AFUNPTR)CallMcSimTxEnd);
  }
   rtn = RTN_FindByName(img, "mcsim_log_begin");
  if (rtn != RTN_Invalid())
  {
    RTN_Replace(rtn, (AFUNPTR)CallMcSimLogBegin);
  }
  rtn = RTN_FindByName(img, "mcsim_log_end");
  if (rtn != RTN_Invalid())
  {
    RTN_Replace(rtn, (AFUNPTR)CallMcSimLogEnd);
  }
  rtn = RTN_FindByName(img, "mcsim_mem_fence");
  if (rtn != RTN_Invalid())
  {
    RTN_Replace(rtn, (AFUNPTR)CallMcSimMemFence);
  }
...
VOID CallMcSimTxBegin()
{
  pthreadsim->mcsim_tx_begin();
}

VOID CallMcSimTxEnd()
{
  pthreadsim->mcsim_tx_end();
}

VOID CallMcSimLogBegin()
{
  pthreadsim->mcsim_log_begin();
}

VOID CallMcSimLogEnd()
{
  pthreadsim->mcsim_log_end();
}

VOID CallMcSimMemFence()
{
  pthreadsim->mcsim_mem_fence();
}
