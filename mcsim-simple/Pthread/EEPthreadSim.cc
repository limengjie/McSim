...
void PthreadSim::initiate(CONTEXT * ctxt)
{
  ...
  {
    scheduler->PlayTraces(trace_name);
    delete scheduler;
    exit(1);
  }
}
...
void PthreadSim::mcsim_tx_begin()
{
  assert(scheduler != NULL);
  if (scheduler->current->second->executed == false)
  {
  ...
  {
    scheduler->mcsim_tx_begin();
  }
}

void PthreadSim::mcsim_tx_end()
{
  assert(scheduler != NULL);
  if (scheduler->current->second->executed == false)
  {
  ...
  {
    scheduler->mcsim_tx_end();
  }
}

void PthreadSim::mcsim_log_begin() // pm baseline 9-4
{
  assert(scheduler != NULL);
  if (scheduler->current->second->executed == false)
  {
  ...
  {
    scheduler->mcsim_log_begin();
  }
}

void PthreadSim::mcsim_log_end() // pm baseline 9-4
{
  assert(scheduler != NULL);
  if (scheduler->current->second->executed == false)
  {
  ...
  {
    scheduler->mcsim_log_end();
  }
}

void PthreadSim::mcsim_mem_fence() // pm baseline 9-4
{
  assert(scheduler != NULL);
  if (scheduler->current->second->executed == false)
  {
