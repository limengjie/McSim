...
class GnuplotCmd:
  ...
  def generate_energydelay(self, datfile_name, epsfile_name, chipkill = False):
    ...
    while i < len(self.ll.class_app_log):
      ...
      cmd = cmd + "set label \"" + class_name + "\" at " + str((self.ll.num_confs_per_app + 1)*(num_apps + num_apps_so_far_per_class - 1)/2) + ", -27\n"
      num_apps_so_far_per_class = num_apps
...
#      cmd = cmd + "set yrange [ 0 : 100 ]\n"
  ...
  def generate_mempower(self, datfile_name, epsfile_name, chipkill = False):
    ...
    if chipkill == False:
      ...
      cmd = cmd + "set yrange [ 0 : 50 ]\n"
      cmd = cmd + "set ytics out 0, 5, 50\n"
  ...
  def generate_edpcomp(self, datfile_name, epsfile_name):
    ...
    while i < len(self.ll.class_app_log):
      ...
      cmd = cmd + "set label \"" + class_name + "\" at " + str((self.ll.num_confs_per_app + 1)*(num_apps + num_apps_so_far_per_class - 1)/2) + ", -4\n"
      num_apps_so_far_per_class = num_apps
